classdef Traces < instrument.ivicom.RepeatedCapability
    ...
    methods
        ...
        function val = Item(obj,Name)
            %ITEM An interface reference pointer to the IAgXSAnSASEMaskTrace interface which is selected by the trace name.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            Name = obj.checkScalarStringArg(Name,'Name');
            try
                [val] = instrument.driver.AgXSAn.DeviceSpecific.SA3.SEMask2.Traces.Item(obj.interface.Item(char(Name)));
            catch e
                obj.interpretError(e);
            end
        end

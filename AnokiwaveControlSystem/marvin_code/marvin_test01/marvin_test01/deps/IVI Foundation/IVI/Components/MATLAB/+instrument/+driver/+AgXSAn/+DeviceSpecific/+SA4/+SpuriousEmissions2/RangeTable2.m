classdef RangeTable2 < instrument.ivicom.Interface
    %RANGETABLE2 IAgXSAnSASpuriousEmissionsRangeTable2 interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = RangeTable2(interface)
            ...
            try
                obj.Start = instrument.driver.AgXSAn.DeviceSpecific.SA4.SpuriousEmissions2.RangeTable2.Start(obj.interface.Start);
            catch %#ok<CTCH>
            end

            try
                obj.Stop = instrument.driver.AgXSAn.DeviceSpecific.SA4.SpuriousEmissions2.RangeTable2.Stop(obj.interface.Stop);
            catch %#ok<CTCH>
            end

            try
                obj.Bandwidth = instrument.driver.AgXSAn.DeviceSpecific.SA4.SpuriousEmissions2.RangeTable2.Bandwidth(obj.interface.Bandwidth);
            catch %#ok<CTCH>
            end
    ...
    properties
        ...
        %SWEEPTYPEAUTOENABLED Sets the sweep type of the spurious measurement to either Auto or Swept. When in Auto, the selections of swept type of ranges are governed by the Best Speed Sweep Type Rule, and FFT analysis might be chosen for some ranges if it speeds up the measurement.

classdef SweepCoupling < instrument.ivicom.Interface
    %SWEEPCOUPLING IAgXSAnSAACPSweepCoupling interface.
    ...
    properties
        ...
        %SWEEPPOINTS The number of points per sweep.
        SweepPoints
        ...
        %CHANNELINTEGRATIONBANDWIDTH The range of integration used in calculating the power in the main channel.
        ChannelIntegrationBandwidth

        %RESOLUTIONBANDWIDTH The 3.01 dB resolution bandwidth (RBW) of the analyzer.
        ResolutionBandwidth

        %RESOLUTIONBANDWIDTHAUTOENABLED Turns resolution bandwidth auto on and off.
        ResolutionBandwidthAutoEnabled

        %BANDWIDTHSHAPE The type of bandwidth filter that is used.
        BandwidthShape

        %BANDWIDTHTYPE The Gaussian filter based on its -3 dB (Normal) bandwidth or its -6 dB bandwidth.
        BandwidthType

        %VIDEOBANDWIDTH The analyzer post-detection filter bandwidth(VBW).
        VideoBandwidth

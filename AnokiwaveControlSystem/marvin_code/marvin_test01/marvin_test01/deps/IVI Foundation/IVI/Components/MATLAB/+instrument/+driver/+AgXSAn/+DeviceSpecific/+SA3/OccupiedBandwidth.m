classdef OccupiedBandwidth < instrument.ivicom.Interface
    ...
    methods (Hidden=true)
        ...
        function obj = OccupiedBandwidth(interface)
            ...
            try
                obj.Traces = instrument.driver.AgXSAn.DeviceSpecific.SA3.OccupiedBandwidth.Traces(obj.interface.Traces);
            catch %#ok<CTCH>
            end

            try
                obj.Average = instrument.driver.AgXSAn.DeviceSpecific.SA3.OccupiedBandwidth.Average(obj.interface.Average);
            catch %#ok<CTCH>
            end

            try
                obj.Frequency = instrument.driver.AgXSAn.DeviceSpecific.SA3.OccupiedBandwidth.Frequency(obj.interface.Frequency);
            catch %#ok<CTCH>
            end

            try
                obj.Display = instrument.driver.AgXSAn.DeviceSpecific.SA3.OccupiedBandwidth.Display(obj.interface.Display);
            catch %#ok<CTCH>
            end

            try
                obj.Marker = instrument.driver.AgXSAn.DeviceSpecific.SA3.OccupiedBandwidth.Marker(obj.interface.Marker);
            catch %#ok<CTCH>
            end

            try
                obj.SweepCoupling = instrument.driver.AgXSAn.DeviceSpecific.SA3.OccupiedBandwidth.SweepCoupling(obj.interface.SweepCoupling);
            catch %#ok<CTCH>
            end

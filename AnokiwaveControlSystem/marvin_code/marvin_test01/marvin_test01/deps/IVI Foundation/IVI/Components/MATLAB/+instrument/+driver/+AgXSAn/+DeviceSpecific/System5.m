classdef System5 < instrument.ivicom.Interface
    %SYSTEM5 IAgXSAnSystem5 interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = System5(interface)
    ...
    properties
        ...
        %FORMAT specifies the format of the trace data input and output.
        Format
    ...
    methods
        ...
        function ModePreset(obj)
            %MODEPRESET Returns the active mode to a known state.
            
            obj.checkValidObj();
			error(nargchk(1,1,nargin,'struct'))
            try
                obj.interface.ModePreset();
            catch e
                obj.interpretError(e);
            end
        end
        ...
        function UserPresetAllModes(obj)
            %USERPRESETALLMODES Recalls all of the User Preset files for each mode, switches to the power-on mode, and activates the savedmeasurement from the power-on mode User Preset file.

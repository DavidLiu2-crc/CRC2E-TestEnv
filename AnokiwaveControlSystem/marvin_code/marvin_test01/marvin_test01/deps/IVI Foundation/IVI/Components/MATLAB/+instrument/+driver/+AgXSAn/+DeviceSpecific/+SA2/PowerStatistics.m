classdef PowerStatistics < instrument.ivicom.Interface
    ...
    methods (Hidden=true)
        ...
        function obj = PowerStatistics(interface)
            ...
            try
                obj.Traces = instrument.driver.AgXSAn.DeviceSpecific.SA2.PowerStatistics.Traces(obj.interface.Traces);
            catch %#ok<CTCH>
            end

            try
                obj.Display = instrument.driver.AgXSAn.DeviceSpecific.SA2.PowerStatistics.Display(obj.interface.Display);
            catch %#ok<CTCH>
            end

            try
                obj.SweepCoupling = instrument.driver.AgXSAn.DeviceSpecific.SA2.PowerStatistics.SweepCoupling(obj.interface.SweepCoupling);
            catch %#ok<CTCH>
            end

            try
                obj.Marker = instrument.driver.AgXSAn.DeviceSpecific.SA2.PowerStatistics.Marker(obj.interface.Marker);
            catch %#ok<CTCH>
            end

classdef Marker2 < instrument.ivicom.Interface
    %MARKER2 IAgXSAnBasicWaveformMarker2 interface.
    ...
    properties
        ...
        %FUNCTIONBANDLEFT The left edge frequency or time for the band of the selected marker. The right edge is unaffected.
        FunctionBandLeft

        %FUNCTIONBANDRIGHT The right edge frequency or time for the band of the selected marker. The left edge is unaffected.
        FunctionBandRight

        %FUNCTIONBANDSPAN The width of the span for the selected marker.
        FunctionBandSpan

        %COUPLEDENABLED Enable or disable the state of the markers to be coupled.
        CoupledEnabled
        ...
        %FUNCTION The marker control mode.
        Function
    ...
    methods
        ...
        %% CoupledEnabled property access methods
        function value = get.CoupledEnabled(obj)
            obj.checkValidObj();
            try
                value = obj.interface.CoupledEnabled;
            catch e
                obj.interpretError(e);
            end
        end
		function set.CoupledEnabled(obj,newValue)
			obj.checkValidObj();
            newValue = obj.checkScalarBoolArg(newValue,'CoupledEnabled');
            try
                obj.interface.CoupledEnabled = newValue;
            catch e
                obj.interpretError(e);
            end
        end

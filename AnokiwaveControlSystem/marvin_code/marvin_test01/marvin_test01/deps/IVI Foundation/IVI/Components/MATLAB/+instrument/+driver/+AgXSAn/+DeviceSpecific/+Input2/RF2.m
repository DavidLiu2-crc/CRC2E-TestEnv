classdef RF2 < instrument.ivicom.Interface
    %RF2 IAgXSAnInputRF2 interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = RF2(interface)
            ...
            try
                obj.Corrections = instrument.driver.AgXSAn.DeviceSpecific.Input2.RF2.Corrections(obj.interface.Corrections);
            catch %#ok<CTCH>
            end
    ...
    properties
        ...
        %RANGE The amplitude of the largest sinusoidal signal.
        Range

        %PEAKTOAVERAGE Range setting to optimize the level control in the instrument.The value is the ratio, in dB, of the peak power to the average power of the signal to be measured.
        PeakToAverage
    ...
    methods
		...
		function set.Range(obj,newValue)
            ...
            try
                obj.interface.Range = newValue;
            catch e
                obj.interpretError(e);
            end
        end

        %% PeakToAverage property access methods
        function value = get.PeakToAverage(obj)
            obj.checkValidObj();
            try
                value = obj.interface.PeakToAverage;
            catch e
                obj.interpretError(e);
            end
        end
		function set.PeakToAverage(obj,newValue)
			obj.checkValidObj();
            newValue = obj.checkScalarDoubleArg(newValue,'PeakToAverage');
            try
                obj.interface.PeakToAverage = newValue;
            catch e
                obj.interpretError(e);
            end
        end

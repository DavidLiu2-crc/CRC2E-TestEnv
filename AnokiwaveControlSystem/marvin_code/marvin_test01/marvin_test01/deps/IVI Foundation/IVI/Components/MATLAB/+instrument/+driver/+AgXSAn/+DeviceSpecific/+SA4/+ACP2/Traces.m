classdef Traces < instrument.ivicom.RepeatedCapability
    ...
    methods
        ...
        function InitiateSignal(obj,Frequency,Amplitude)
            %INITIATESIGNAL This method set the centre frequency, amplitude of the signal and initiates a trigger cycle for the measurement specified.
        ...
        function val = Item(obj,Name)
            %ITEM An interface reference pointer to the IAgXSAnSAACPTrace interface which is selected by the trace name.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            Name = obj.checkScalarStringArg(Name,'Name');
            try
                [val] = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP2.Traces.Item(obj.interface.Item(char(Name)));
            catch e
                obj.interpretError(e);
            end
        end

classdef Traces < instrument.ivicom.RepeatedCapability
    %TRACES AgXSAnBasic SpectrumTrace collection interface.
    ...
    properties (SetAccess = private)
        ...
        %COUNT ReadOnly The number of traces. Read Only.
        Count
    ...
    methods
        ...
        function Initiate(obj)
            %INITIATE This command initiates a trigger cycle for the measurement specified.
        ...
        function val = Item(obj,Name)
            %ITEM An interface reference pointer to the IAgXSAnBasicSpectrumTrace interface which is selected by the trace name.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            Name = obj.checkScalarStringArg(Name,'Name');
            try
                [val] = instrument.driver.AgXSAnBasic.DeviceSpecific.Spectrum2.Traces.Item(obj.interface.Item(char(Name)));
            catch e
                obj.interpretError(e);
            end
        end
        ...
        function val = Name(obj,Index)
            %NAME ReadOnly The trace name for a given index.

classdef Carrier3 < instrument.ivicom.Interface
    %CARRIER3 IAgXSAnSAACPCarrier3 interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = Carrier3(interface)
            ...
            try
                obj.List3 = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier3.List3(obj.interface.List3);
            catch %#ok<CTCH>
            end

            try
                obj.List2 = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier3.List2(obj.interface.List2);
            catch %#ok<CTCH>
            end

            try
                obj.List = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier3.List(obj.interface.List);
            catch %#ok<CTCH>
            end
    ...
    properties
        ...
        %REFERENCETYPE Selects the power reference type
        ReferenceType
    ...
    properties (SetAccess = private)
        ...
        %LIST3 Pointer to IAgXSAnSAACPCarrierList interface. Read Only.
        List3

        %LIST2 Pointer to IAgXSAnSAACPCarrierList interface. Read Only.
        List2
    ...
    methods
        ...
        %% List3 property access methods
        function value = get.List3(obj)
            obj.checkValidObj();
            if isempty(obj.List3)
                try
                    obj.List3 = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier3.List3(obj.interface.List3);
                catch e
        ...
        %% ReferenceType property access methods
        function value = get.ReferenceType(obj)
            obj.checkValidObj();
            try
                value = obj.interface.ReferenceType;
            catch e
                obj.interpretError(e);
            end
        end
		function set.ReferenceType(obj,newValue)
			obj.checkValidObj();
            try
                obj.interface.ReferenceType = newValue;
            catch e
                obj.interpretError(e);
            end
        end

classdef Marker < instrument.ivicom.Interface
    %MARKER IAgXSAnSABurstPowerMarker interface.
    ...
    properties
        ...
        %POSITION Sets the marker X position to a specified point on the X axis in the current X-axis units (frequency or time).  If MarkerFrequencyCounter.Enabled is true,  Position query returns the frequency counter reading at the current point.  Marker must be enabled.
        Position

        %TYPE Sets the control mode of the active marker.
        Type
        ...
        %TRACEPOINTSPOSITION Sets the marker X position in trace points.
        TracePointsPosition
    ...
    properties (SetAccess = private)
        ...
        %AMPLITUDE Queries the marker Y value or delta in the current y axis units.  If Marker.Function is on it returns the result of the selected marker function.  Marker must be enabled. Read Only.
        Amplitude
    ...
    methods
        ...
        function DisableAll(obj)
            %DISABLEALL Turns off all markers, including markers used for signal track.
        ...
        function Search(obj,SearchType)
            %SEARCH Places the marker according to the search type specified.

classdef HarmonicDistortion < instrument.ivicom.Interface
    %HARMONICDISTORTION IAgXSAnSAHarmonicDistortion interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = HarmonicDistortion(interface)
            ...
            try
                obj.Traces = instrument.driver.AgXSAn.DeviceSpecific.SA4.HarmonicDistortion.Traces(obj.interface.Traces);
            catch %#ok<CTCH>
            end

            try
                obj.Average = instrument.driver.AgXSAn.DeviceSpecific.SA4.HarmonicDistortion.Average(obj.interface.Average);
            catch %#ok<CTCH>
            end

            try
                obj.SweepCoupling = instrument.driver.AgXSAn.DeviceSpecific.SA4.HarmonicDistortion.SweepCoupling(obj.interface.SweepCoupling);
            catch %#ok<CTCH>
            end

            try
                obj.RangeTable = instrument.driver.AgXSAn.DeviceSpecific.SA4.HarmonicDistortion.RangeTable(obj.interface.RangeTable);
            catch %#ok<CTCH>
            end

            try
                obj.Tone = instrument.driver.AgXSAn.DeviceSpecific.SA4.HarmonicDistortion.Tone(obj.interface.Tone);
            catch %#ok<CTCH>
            end

            try
                obj.DisplayWindowY = instrument.driver.AgXSAn.DeviceSpecific.SA4.HarmonicDistortion.DisplayWindowY(obj.interface.DisplayWindowY);
            catch %#ok<CTCH>
            end

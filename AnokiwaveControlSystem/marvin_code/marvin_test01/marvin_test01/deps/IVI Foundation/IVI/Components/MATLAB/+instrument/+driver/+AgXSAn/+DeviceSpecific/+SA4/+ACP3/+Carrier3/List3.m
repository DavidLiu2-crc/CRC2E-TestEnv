classdef List3 < instrument.ivicom.Interface
    %LIST3 IAgXSAnSAACPCarrierList3 interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = List3(interface)
    ...
    methods
        ...
        function val = QueryAlphaValue(obj)
            %QUERYALPHAVALUE Gets the Alpha values for the filter used in the current carrier configuration.
            ...
            try
                ...
                [val] = obj.interface.QueryAlphaValue();
            catch e
        ...
        function ConfigureAlphaValue(obj,AlphaValues)
            %CONFIGUREALPHAVALUE Sets the Alpha values for the filter used in the current carrier configuration.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            AlphaValues = obj.checkVectorDoubleArg(AlphaValues,'AlphaValues');
            singleDimFlag = feature('COM_SafeArraySingleDim');
            try
                feature('COM_SafeArraySingleDim',1);
                obj.interface.ConfigureAlphaValue(double(AlphaValues));
            catch e
                feature('COM_SafeArraySingleDim',singleDimFlag);
                obj.interpretError(e);
            end

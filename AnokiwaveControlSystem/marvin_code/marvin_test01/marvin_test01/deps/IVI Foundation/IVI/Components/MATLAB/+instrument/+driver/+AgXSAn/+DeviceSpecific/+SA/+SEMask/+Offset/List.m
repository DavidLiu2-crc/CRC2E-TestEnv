classdef List < instrument.ivicom.Interface
    ...
    methods (Hidden=true)
        ...
        function obj = List(interface)
            ...
            try
                obj.Bandwidth = instrument.driver.AgXSAn.DeviceSpecific.SA.SEMask.Offset.List.Bandwidth(obj.interface.Bandwidth);
            catch %#ok<CTCH>
            end

            try
                obj.Start = instrument.driver.AgXSAn.DeviceSpecific.SA.SEMask.Offset.List.Start(obj.interface.Start);
            catch %#ok<CTCH>
            end

            try
                obj.Stop = instrument.driver.AgXSAn.DeviceSpecific.SA.SEMask.Offset.List.Stop(obj.interface.Stop);
            catch %#ok<CTCH>
            end

classdef RangeTable < instrument.ivicom.Interface
    ...
    methods (Hidden=true)
        ...
        function obj = RangeTable(interface)
            ...
            try
                obj.Start = instrument.driver.AgXSAn.DeviceSpecific.SA.SpuriousEmissions.RangeTable.Start(obj.interface.Start);
            catch %#ok<CTCH>
            end

            try
                obj.Stop = instrument.driver.AgXSAn.DeviceSpecific.SA.SpuriousEmissions.RangeTable.Stop(obj.interface.Stop);
            catch %#ok<CTCH>
            end

            try
                obj.Bandwidth = instrument.driver.AgXSAn.DeviceSpecific.SA.SpuriousEmissions.RangeTable.Bandwidth(obj.interface.Bandwidth);
            catch %#ok<CTCH>
            end

classdef SpuriousEmissions < instrument.ivicom.Interface
    ...
    methods (Hidden=true)
        ...
        function obj = SpuriousEmissions(interface)
            ...
            try
                obj.Traces = instrument.driver.AgXSAn.DeviceSpecific.SA3.SpuriousEmissions.Traces(obj.interface.Traces);
            catch %#ok<CTCH>
            end

            try
                obj.Average = instrument.driver.AgXSAn.DeviceSpecific.SA3.SpuriousEmissions.Average(obj.interface.Average);
            catch %#ok<CTCH>
            end

            try
                obj.Display = instrument.driver.AgXSAn.DeviceSpecific.SA3.SpuriousEmissions.Display(obj.interface.Display);
            catch %#ok<CTCH>
            end

            try
                obj.Marker = instrument.driver.AgXSAn.DeviceSpecific.SA3.SpuriousEmissions.Marker(obj.interface.Marker);
            catch %#ok<CTCH>
            end

            try
                obj.RangeTable = instrument.driver.AgXSAn.DeviceSpecific.SA3.SpuriousEmissions.RangeTable(obj.interface.RangeTable);
            catch %#ok<CTCH>
            end

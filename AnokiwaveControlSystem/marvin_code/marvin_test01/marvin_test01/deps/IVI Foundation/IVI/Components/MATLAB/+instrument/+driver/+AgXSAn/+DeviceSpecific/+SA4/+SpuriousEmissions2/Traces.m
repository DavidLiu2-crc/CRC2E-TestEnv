classdef Traces < instrument.ivicom.RepeatedCapability
    ...
    methods
        ...
        function val = Item(obj,Name)
            %ITEM An interface reference pointer to the IAgXSAnSASpuriousEmissionsTrace interface which is selected by the trace name.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            Name = obj.checkScalarStringArg(Name,'Name');
            try
                [val] = instrument.driver.AgXSAn.DeviceSpecific.SA4.SpuriousEmissions2.Traces.Item(obj.interface.Item(char(Name)));
            catch e
                obj.interpretError(e);
            end
        end

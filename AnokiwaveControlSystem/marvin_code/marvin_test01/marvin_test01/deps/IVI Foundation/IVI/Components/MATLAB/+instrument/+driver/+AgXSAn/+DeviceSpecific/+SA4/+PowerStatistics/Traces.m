classdef Traces < instrument.ivicom.RepeatedCapability
    ...
    methods
        ...
        function StoreAsReference(obj)
            %STOREASREFERENCE Copies the currently measured curve as the user-definable reference trace. The captured data remains until the other mode is chosen.
            
            obj.checkValidObj();
			error(nargchk(1,1,nargin,'struct'))
            try
                obj.interface.StoreAsReference();
            catch e
                obj.interpretError(e);
            end
        end
        ...
        function val = Item(obj,Name)
            %ITEM An interface reference pointer to the IAgXSAnSAPowerStatisticsTrace interface which is selected by the trace name.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            Name = obj.checkScalarStringArg(Name,'Name');
            try
                [val] = instrument.driver.AgXSAn.DeviceSpecific.SA4.PowerStatistics.Traces.Item(obj.interface.Item(char(Name)));
            catch e
                obj.interpretError(e);
            end
        end

classdef Trigger < instrument.ivicom.Interface
    %TRIGGER IVI SpecAn class-compliant trigger interface
    ...
    methods (Hidden=true)
        ...
        function obj = Trigger(interface)
            ...
            try
                obj.External = instrument.driver.AgXSAn.DeviceSpecific.Trigger.External(obj.interface.External);
            catch %#ok<CTCH>
            end

            try
                obj.Video = instrument.driver.AgXSAn.DeviceSpecific.Trigger.Video(obj.interface.Video);
            catch %#ok<CTCH>
            end

            try
                obj.Sources = instrument.driver.AgXSAn.DeviceSpecific.Trigger.Sources(obj.interface.Sources);
            catch %#ok<CTCH>
            end

            try
                obj.Alarms = instrument.driver.AgXSAn.DeviceSpecific.Trigger.Alarms(obj.interface.Alarms);
            catch %#ok<CTCH>
            end

            try
                obj.External2 = instrument.driver.AgXSAn.DeviceSpecific.Trigger.External2(obj.interface.External2);
            catch %#ok<CTCH>
            end

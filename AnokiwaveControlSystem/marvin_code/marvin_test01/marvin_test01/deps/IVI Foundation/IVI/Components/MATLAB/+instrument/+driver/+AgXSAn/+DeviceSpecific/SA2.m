classdef SA2 < instrument.ivicom.Interface
    %SA2 IAgXSAnSA2 interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = SA2(interface)
            ...
            try
                obj.Source = instrument.driver.AgXSAn.DeviceSpecific.SA2.Source(obj.interface.Source);
            catch %#ok<CTCH>
            end

            try
                obj.ACP = instrument.driver.AgXSAn.DeviceSpecific.SA2.ACP(obj.interface.ACP);
            catch %#ok<CTCH>
            end

            try
                obj.BurstPower = instrument.driver.AgXSAn.DeviceSpecific.SA2.BurstPower(obj.interface.BurstPower);
            catch %#ok<CTCH>
            end

            try
                obj.ChannelPower = instrument.driver.AgXSAn.DeviceSpecific.SA2.ChannelPower(obj.interface.ChannelPower);
            catch %#ok<CTCH>
            end

            try
                obj.OccupiedBandwidth = instrument.driver.AgXSAn.DeviceSpecific.SA2.OccupiedBandwidth(obj.interface.OccupiedBandwidth);
            catch %#ok<CTCH>
            end

            try
                obj.PowerStatistics = instrument.driver.AgXSAn.DeviceSpecific.SA2.PowerStatistics(obj.interface.PowerStatistics);
            catch %#ok<CTCH>
            end

            try
                obj.SEMask = instrument.driver.AgXSAn.DeviceSpecific.SA2.SEMask(obj.interface.SEMask);
            catch %#ok<CTCH>
            end

            try
                obj.SpuriousEmissions = instrument.driver.AgXSAn.DeviceSpecific.SA2.SpuriousEmissions(obj.interface.SpuriousEmissions);
            catch %#ok<CTCH>
            end

            try
                obj.HarmonicDistortion = instrument.driver.AgXSAn.DeviceSpecific.SA2.HarmonicDistortion(obj.interface.HarmonicDistortion);
            catch %#ok<CTCH>
            end

            try
                obj.ThirdOrderIntercept = instrument.driver.AgXSAn.DeviceSpecific.SA2.ThirdOrderIntercept(obj.interface.ThirdOrderIntercept);
            catch %#ok<CTCH>
            end

            try
                obj.Gating = instrument.driver.AgXSAn.DeviceSpecific.SA2.Gating(obj.interface.Gating);
            catch %#ok<CTCH>
            end

            try
                obj.Limits = instrument.driver.AgXSAn.DeviceSpecific.SA2.Limits(obj.interface.Limits);
            catch %#ok<CTCH>
            end

            try
                obj.ListSweep = instrument.driver.AgXSAn.DeviceSpecific.SA2.ListSweep(obj.interface.ListSweep);
            catch %#ok<CTCH>
            end

            try
                obj.Marker = instrument.driver.AgXSAn.DeviceSpecific.SA2.Marker(obj.interface.Marker);
            catch %#ok<CTCH>
            end

            try
                obj.SweepCoupling = instrument.driver.AgXSAn.DeviceSpecific.SA2.SweepCoupling(obj.interface.SweepCoupling);
            catch %#ok<CTCH>
            end

            try
                obj.SweptSAs = instrument.driver.AgXSAn.DeviceSpecific.SA2.SweptSAs(obj.interface.SweptSAs);
            catch %#ok<CTCH>
            end

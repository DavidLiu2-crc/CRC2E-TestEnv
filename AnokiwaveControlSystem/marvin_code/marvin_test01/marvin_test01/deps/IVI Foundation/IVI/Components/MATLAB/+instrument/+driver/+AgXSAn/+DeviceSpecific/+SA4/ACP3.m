classdef ACP3 < instrument.ivicom.Interface
    %ACP3 IAgXSAnSAACP3 interface.
    
    %% Construction/Clean up
    methods (Hidden=true)

        function obj = ACP3(interface)
            ...
            try
                obj.Carrier3 = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier3(obj.interface.Carrier3);
            catch %#ok<CTCH>
            end

            try
                obj.Carrier2 = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier2(obj.interface.Carrier2);
            catch %#ok<CTCH>
            end

            try
                obj.Traces = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Traces(obj.interface.Traces);
            catch %#ok<CTCH>
            end

            try
                obj.Average = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Average(obj.interface.Average);
            catch %#ok<CTCH>
            end

            try
                obj.Carrier = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier(obj.interface.Carrier);
            catch %#ok<CTCH>
            end

            try
                obj.Display = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Display(obj.interface.Display);
            catch %#ok<CTCH>
            end

            try
                obj.Frequency = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Frequency(obj.interface.Frequency);
            catch %#ok<CTCH>
            end

            try
                obj.Marker = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Marker(obj.interface.Marker);
            catch %#ok<CTCH>
            end

            try
                obj.SweepCoupling = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.SweepCoupling(obj.interface.SweepCoupling);
            catch %#ok<CTCH>
            end

            try
                obj.Offset = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Offset(obj.interface.Offset);
            catch %#ok<CTCH>
            end
    ...
    properties (SetAccess = private)
        ...
        %CARRIER3 Pointer to IAgXSAnSAACPCarrier interface. Read Only.
        Carrier3

        %CARRIER2 Pointer to IAgXSAnSAACPCarrier interface. Read Only.
        Carrier2
    ...
    methods
        ...
        %% Carrier3 property access methods
        function value = get.Carrier3(obj)
            obj.checkValidObj();
            if isempty(obj.Carrier3)
                try
                    obj.Carrier3 = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier3(obj.interface.Carrier3);
                catch e
        ...
        %% Carrier2 property access methods
        function value = get.Carrier2(obj)
            obj.checkValidObj();
            if isempty(obj.Carrier2)
                try
                    obj.Carrier2 = instrument.driver.AgXSAn.DeviceSpecific.SA4.ACP3.Carrier2(obj.interface.Carrier2);
                catch e
                    obj.interpretError(e);

classdef DriverOperation < instrument.ivicom.Interface
    ...
    properties
        ...
        %CACHE Drivers may choose to always cache some instrument settings, never cache others, and optionally cache others, to avoid unnecessary I/O to the instrument.  If True, the driver caches optionally cached instrument settings.
        Cache

        %INTERCHANGECHECK Not Supported - If True, the driver maintains a record of interchangeability warnings.  If the driver does not support interchangeability checking, attempts to set InterchangeCheck to True return an error.
        InterchangeCheck
        ...
        %RECORDCOERCIONS Not Supported - If True, the driver keeps a list of the value coercions it makes for ViInt32 and ViReal64 attributes.  If the driver does not support coercion recording, attempts to set RecordCoercions to True will return an error.
        RecordCoercions
    ...
    methods
        ...
        function ClearInterchangeWarnings(obj)
            %CLEARINTERCHANGEWARNINGS Not Supported - Clears the list of interchangeability warnings that the IVI specific driver maintains.
        ...
        function val = GetNextInterchangeWarning(obj)
            %GETNEXTINTERCHANGEWARNING Not Supported - Returns the oldest warning from the interchange warning list.  Records are only added to the list if InterchangeCheck is True.
        ...
        function ResetInterchangeCheck(obj)
            %RESETINTERCHANGECHECK Not Supported - Resets the interchangeability checking algorithms of the driver so that methods and properties that executed prior to calling this function have no affect on whether future calls to the driver generate interchangeability warnings.
        ...
        function val = GetNextCoercionRecord(obj)
            %GETNEXTCOERCIONRECORD Not Supported - Returns the oldest record from the coercion record list.  Records are only added to the list if RecordCoercions is True.

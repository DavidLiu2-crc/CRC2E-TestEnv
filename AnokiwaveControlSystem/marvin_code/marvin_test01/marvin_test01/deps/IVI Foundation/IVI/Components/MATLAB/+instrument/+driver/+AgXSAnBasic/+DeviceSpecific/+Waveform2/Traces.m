classdef Traces < instrument.ivicom.RepeatedCapability
    %TRACES AgXSAnBasic WaveformTrace collection interface.
    ...
    methods
        ...
        function val = Item(obj,Name)
            %ITEM An interface reference pointer to the IAgXSAnBasicWaveformTrace interface which is selected by the trace name.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            Name = obj.checkScalarStringArg(Name,'Name');
            try
                [val] = instrument.driver.AgXSAnBasic.DeviceSpecific.Waveform2.Traces.Item(obj.interface.Item(char(Name)));
            catch e
                obj.interpretError(e);
            end
        end

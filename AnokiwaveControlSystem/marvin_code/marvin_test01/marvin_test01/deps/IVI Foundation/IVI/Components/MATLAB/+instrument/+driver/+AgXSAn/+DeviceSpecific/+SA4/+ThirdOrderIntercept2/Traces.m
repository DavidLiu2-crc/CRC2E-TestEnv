classdef Traces < instrument.ivicom.RepeatedCapability
    ...
    properties
        ...
        %TYPE Get or Set selection of trace max hold characteristics.
        Type
    ...
    methods
        ...
        function val = Item(obj,Name)
            %ITEM An interface reference pointer to the IAgXSAnSAThirdOrderInterceptTrace interface which is selected by the trace name.
            
            obj.checkValidObj();
			error(nargchk(2,2,nargin,'struct'))
            Name = obj.checkScalarStringArg(Name,'Name');
            try
                [val] = instrument.driver.AgXSAn.DeviceSpecific.SA4.ThirdOrderIntercept2.Traces.Item(obj.interface.Item(char(Name)));
            catch e
                obj.interpretError(e);
            end
        end

...
typedef struct 
{
	...
	unsigned short endpointIndex;			/* positive number, usually corresponding to the funcNumber */
	unsigned short manufId;
	unsigned short modelCode;
	char           manufName[256];
	char           modelName[256];
	char           slotPath[256];
	char		   chassisAddress[256];
	...
	short          supportedByKtVisa;
	unsigned short isPrimary;				/* TRUE(1), FALSE(0). This is of importance for multi-endpoint modules.*/
	char		   occupiedSlotList[50];	
	short          reserved3;
	int            reserved2;
	int            reserved1;
} pxiinfo2;
